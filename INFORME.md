...
- Dentro de `Sum`, en lugar de realizar un *broadcast* de los datos que enviaría cada fruta a todas las instancias de `Aggregation` (lo que generaría procesamiento redundante y fragmentado), implementé un **particionado por clave (*sharding*)**. Para evitar los problemas de hashes aleatorios del `hash()` nativo entre procesos en Python, se utilizó la librería determinística `hashlib`. Esto garantiza que una fruta en particular sea derivada **siempre al mismo nodo `Aggregation`**, sin importar cuál instancia de `Sum` la haya procesado.
